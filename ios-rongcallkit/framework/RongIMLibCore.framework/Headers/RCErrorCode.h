...
typedef NS_ENUM(NSInteger, RCErrorCode) {
    ...
    /// 用户被订阅量达到上限，详细查看开发文档。
    ///
    /// - Solution 比如 A/B/C 等很多用户都订阅了用户 X，如果再有 a/b/c 等继续订阅 X，用户 X 的被订阅量超过上限
    ...
    /// 未知的错误，http 状态码会是 500 未知的错误，http 状态码会是 500
    ///
    ...
    /// 文件已过期或被清理 小视频文件默认存储 7 天，其它文件默认存储 6 个月。到期后自动清理
    ///
    ...
    /// 开发者调用接口传入的 MessageTag 为空 或者 MessageTag.value() 为空或者 ""
    ///

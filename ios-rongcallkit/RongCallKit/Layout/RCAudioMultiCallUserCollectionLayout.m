...
- (CGFloat)itemAreaWidth {
  if (!_itemAreaWidth) {
    ...
    _itemAreaWidth = MIN((frame.size.width / 4),
                         (frame.size.height - self.buttomPadding) / 2);
  }
...
- (void)prepareLayout {
  ...
  if (itemCount <= 8) {
    ...
    CGFloat topY = (frame.size.height - self.buttomPadding) / 2 -
                   self.itemAreaWidth +
                   (topCount == itemCount ? self.itemAreaWidth / 2 : 0);
    CGFloat leftX1 =
        (frame.size.width - self.itemAreaWidth * topCount) / 2; // line1
    CGFloat leftX2 =
        (frame.size.width - self.itemAreaWidth * (itemCount - topCount)) /
        2; // line2
    for (int i = 0; i < itemCount; i++) {
      ...
      if (i < topCount) {
        attributes.frame =
            CGRectMake(leftX1 + self.itemAreaWidth * i + self.itemMargin,
                       topY + self.itemMargin, self.itemWidth, self.itemWidth);
      } else {
        attributes.frame = CGRectMake(
            leftX2 + self.itemAreaWidth * (i - topCount) + self.itemMargin,
            topY + self.itemMargin + self.itemWidth + self.itemMargin +
                self.itemMargin,
    ...
    CGFloat topY =
        (frame.size.height - self.buttomPadding) / 2 - self.itemAreaWidth;
            ...
            [UICollectionViewLayoutAttributes
                layoutAttributesForCellWithIndexPath:[NSIndexPath
                                                         indexPathForItem:index
                                                                inSection:0]];
        attributes.frame =
            CGRectMake((frame.size.width - self.itemAreaWidth * maxCloumn) / 2 +
                           self.itemAreaWidth * cloumn + self.itemMargin,
                       topY + self.itemAreaWidth * line + self.itemMargin,
                       self.itemWidth, self.itemWidth);
        [self.attributesArray addObject:attributes];
        NSLog(@"the index is %d", index);
        NSLog(@"frame is %f, %f, %f, %f", attributes.frame.origin.x,
              attributes.frame.origin.y, attributes.frame.size.width,
...
- (CGSize)collectionViewContentSize {
  if ([self.collectionView numberOfItemsInSection:0] <= 8) {
    ...
    int lines =
        ((int)[self.collectionView numberOfItemsInSection:0] - 1) / 4 + 1;
    CGFloat height = [self collectionView].frame.size.height +
                     self.itemAreaWidth * (lines - 2) - self.buttomPadding;

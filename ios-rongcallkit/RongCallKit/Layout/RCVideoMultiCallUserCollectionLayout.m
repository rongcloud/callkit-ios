    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (i < 4) {
        attributes.frame = CGRectMake(
            leftMargin + (3 - i) * self.itemAreaWidth + self.itemMargin,
            topY + self.itemMargin, self.itemWidth, self.itemWidth);
      } else {
        attributes.frame = CGRectMake(
            leftMargin + (7 - i) * self.itemAreaWidth + self.itemMargin,
            topY - self.itemAreaWidth + self.itemMargin, self.itemWidth,
            self.itemWidth);
      }

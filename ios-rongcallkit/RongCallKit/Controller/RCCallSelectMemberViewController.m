...
@interface RCCallSelectMemberViewController () <UITableViewDataSource,
                                                UITableViewDelegate>
...
- (void)viewDidLoad {
  ...
  self.tableView.frame = CGRectMake(0, 64, self.view.frame.size.width,
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  RCCallSelectingMemberCell *cell =
      [tableView dequeueReusableCellWithIdentifier:reusableID];
  if (!cell) {
    cell = [[RCCallSelectingMemberCell alloc]
  ...
  if ([self.existUserIdList containsObject:userId]) {
    [cell.selectedImageView
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/deselect.png"]];
  } else if ([self.selectUserIds containsObject:userId]) {
    [cell.selectedImageView
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/selected.png"]];
  } else {
    [cell.selectedImageView
        setImage:[RCCallKitUtility imageFromVoIPBundle:@"voip/unselected.png"]];
  }
    ...
    if ([updateUserId isEqualToString:userId]) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        NSIndexPath *index = [NSIndexPath
            indexPathForRow:[weakSelf.listingUserIdList indexOfObject:userId]
                  inSection:0];

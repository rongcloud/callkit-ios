...
- (UIButton *)confirmButton {
    if (!_confirmButton) {
        _confirmButton =
            [[UIButton alloc] initWithFrame:CGRectMake(UIScreen.mainScreen.bounds.size.width - 80, 17, 80.0, 30.0)];
        [_confirmButton setTitle:RCCallKitLocalizedString(@"Confirm") forState:UIControlStateNormal];
...
- (void)configUI {
    ...
    self.numberLabel.frame =
        CGRectMake(15.0, (self.frame.size.height - 30.0) / 2, UIScreen.mainScreen.bounds.size.width / 2, 30.0);
    ...
    self.confirmButton.frame =
        CGRectMake(UIScreen.mainScreen.bounds.size.width - 80, (self.frame.size.height - 30.0) / 2, 80.0, 30.0);
